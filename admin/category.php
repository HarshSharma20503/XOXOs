...
if(isset($_GET['type']) && $_GET['type']!=='' && isset($_GET['id']) && $_GET['id']>0){ //checks weather type or id is present in url or not
	$type=get_safe_value($_GET['type']); //gets type and id from url
	$id=get_safe_value($_GET['id']);
	if($type=='delete'){   // condition for delete
		mysqli_query($con,"delete from category where id='$id'");
		redirect('category.php');
	}
	if($type=='active' || $type=='deactive'){ //condition for active or deactive
		$status=1;
		if($type=='deactive'){
			$status=0;
		}
		mysqli_query($con,"update category set status='$status' where id='$id'"); //query to update status
		redirect('category.php');
	}

}

$sql="select * from category order by order_number";  // query to get all category
