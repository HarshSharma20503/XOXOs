...
$msg="";
if(isset($_POST['submit'])){  //checks weather the form is submited or not
	$username=get_safe_value($_POST['username']);  // gets username from using post method
	$password=get_safe_value($_POST['password']);  //gets password from using post method
	
	$sql="select * from admin where username='$username' and password='$password'";  //query to get all the data with matching credantials
	...
	if(mysqli_num_rows($res)>0){     // if number of rows is greater than 1 means user is present
		...
		$_SESSION['ADMIN_USER']=$row['name']; //declares  session variable to store admin name
		redirect('index.php');    //redirect to index.php page
	}else{
