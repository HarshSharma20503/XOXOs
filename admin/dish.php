...
if(isset($_GET['type']) && $_GET['type']!=='' && isset($_GET['id']) && $_GET['id']>0){
	$type=get_safe_value($_GET['type']); //gets the type from url  
	$id=get_safe_value($_GET['id']);     //gets the id from the  url
	if($type=='active' || $type=='deactive'){
		$status=1;
		if($type=='deactive'){
			$status=0;
		}
		mysqli_query($con,"update dish set status='$status' where id='$id'");   //query to update the status
		redirect('dish.php');
	}

}

//this query uses join to get the category name which matches the id of the category 
