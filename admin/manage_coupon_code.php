...
//variables to store all the data fetched from the database
$msg="";
$coupon_code="";
$coupon_type="";
$coupon_value="";
...
if(isset($_GET['id']) && $_GET['id']>0){
	...
	$row=mysqli_fetch_assoc(mysqli_query($con,"select * from coupon_code where id='$id'")); //query to fetch the data on the basis of id
...
if(isset($_POST['submit'])){
	...
	if($id==''){  //condition to check weather the page is logged in for adding or editing
	...
	if(mysqli_num_rows(mysqli_query($con,$sql))>0){   //condition for checking if the coupon code is already present
                  ...
                  <form class="forms-sample" method="post">
					...
					<div class="form-group">
                      ...
                      <select name="coupon_type" required class="form-control">
						...
						foreach($arr as $key=>$val){   //loop to display all the option available in the dropdown menu
							if($key==$coupon_type){    //checks weather we are here for editing or adding
								echo "<option value='".$key."' selected>".$val."</option>";  //display the current key and val
							}else{
